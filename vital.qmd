...
&\text{Lee-Carter model:} & \log(m_{x,t}) &= a_x + k_t b_x + \varepsilon_{x,t} \\[0.3cm]
&\text{Functional data model:}\hspace*{0.3cm} & \log(m_{x,t}) &= \mu(x) + \sum_{j=1}^J \beta_{j,t} \phi_j(x) + \varepsilon_{t}(x)
\end{align*}
where $\varepsilon\sim$ white noise with variance a function of $x$.
\end{block}
...
```{r}
#| label: forecast3
fc |> filter(State == "NSW", .model == "lc") |> autoplot() + scale_y_log10()
```
